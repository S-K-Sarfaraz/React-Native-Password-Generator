...
const PasswordSchema = Yup.object().shape({
  ...
  .min(6, 'Password length should not to lower then 6')
  .max(20, 'Password length should not to greater then 20')
...
const App = () => {
  ...
  const gerneratedPasswordString = (passwordLength: number) => {
    let characterList = '';
    const upperCaseChars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
    const lowerCaseChars = 'abcdefghijklmnopqrstuvwxyz';
    const degitsChars = '0123456789';
    const symbolChars = '!@#$%^&*()_-+=';
      ...
      <SafeAreaView style={styles.appContainer}>
        <View style={styles.formContainer}>
          ...
          <Formik
            ...
            onSubmit={ values => {
              console.log(values)
              gerneratedPasswordString(Number(values.passwordLength))
            }}
          >
            {({
              values,
